...
&=&\frac{(\sqrt{\pi})^{NK}}{\sqrt{det(B)}}exp(\frac{1}{4}a^T B^{-1} a)\\
&=&exp(\frac{1}{4}a^T B^{-1} a+\frac{NK}{2}log(\pi)-\frac{log(det(B))}{2})
\end{eqnarray*}
...
This function is convex in a($w_n$)(if inverse of a +ve semidefinite matrix is always semi-definite). It is not obvious whether it is convex in B($w_e$)

\subsection{Training}
Training boils down to a QP in y.
